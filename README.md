...
- 展开查看完整命令行、可执行文件路径、工作目录、启动时间、用户和父进程
...
- 结束进程带行内确认；支持右键 SIGKILL
- root、系统进程和 launchd 托管进程提供保护提示
...
macOS 可能限制普通应用读取其他用户或系统进程的 socket。RunStat 不会绕过系统安全机制；如果系统仍无法提供数据，会在界面中显示扫描错误，而不是将结果误显示为空列表。
...
- 仅允许结束当前用户拥有的进程。
- root / 系统进程不会直接提供结束权限，并显示保护提示。
- launchd 托管进程结束后可能自动重启，界面会提前提示。
- SIGKILL 仅通过右键菜单提供，并保留与普通结束相同的权限限制。
